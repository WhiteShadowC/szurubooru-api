...
export class PostApi extends SzuruBaseApi {
  async getPosts<F extends keyof Post = keyof Post>(
      args: SzuruRequest<Post, F> & PagedSearchResultQuery
  ): Promise<PagedSearchResult<Post, F>> {
  ...
  async ratePost<F extends keyof Post = keyof Post>(args: SzuruEndpointArgs<'id'> & SzuruPayload<{ score: -1 | 0 | 1 }>): Promise<SzuruResponse<Post, F>> {
    return await this.request('PUT', `post/${args.id}/score`, args);
  }

  async favoritePost<F extends keyof Post = keyof Post>(args: SzuruEndpointArgs<'id'>): Promise<SzuruResponse<Post, F>> {
    return await this.request('POST', `post/${args.id}/favorite`, args);
  }

  async unfavoritePost<F extends keyof Post = keyof Post>(args: SzuruEndpointArgs<'id'>): Promise<SzuruResponse<Post, F>> {
    return await this.request('DELETE', `post/${args.id}/favorite`, args);
  }

  async getFeaturedPost<F extends keyof Post = keyof Post>(): Promise<SzuruResponse<Post, F> | null> {
    return await this.request('GET', `featured-post`, {});
  }

  async setFeaturedPost<F extends keyof Post = keyof Post>(args: SzuruPayload<{ id: number }>): Promise<SzuruResponse<Post, F>> {
    return await this.request('POST', `featured-post`, {});
  }

  async reverseImageSearch<F extends keyof Post = keyof Post>(args: SzuruUpload<'content'>): Promise<ImageSearchResult<F>> {
