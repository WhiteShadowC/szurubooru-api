...
export class UserApi extends SzuruBaseApi {
  async getUsers<F extends keyof User>(args: SzuruRequest<User, F> & SzuruQuery<PagedSearchResultQuery>): Promise<PagedSearchResult<User>> {
    return await this.request('GET', 'users', args);
  }

  async createUser<F extends keyof User>(args: SzuruRequest<User, F> & CreateUserDTO & Partial<SzuruUpload<'avatar'>>): Promise<User> {
  ...
  async getUser<F extends keyof User>(args: SzuruRequest<User, F> & SzuruEndpointArgs<Pick<User, 'name'>>): Promise<User> {
